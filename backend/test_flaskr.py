...
class TriviaTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.database_path = "postgres://{}:{}@{}/{}".format(
            "rashmi", "rashmi", "localhost:5432", self.database_name
        )
    ...
    def test_422_not_all_data_provided_for_post_question(self):
        res = self.client()
        .post("/questions", json=self.new_question_missing_data)
        data = json.loads(res.data)
    ...
    def test_405_not_all_data_provided_for_post_question(self):
        res = self.client()
        .post("/questions/1", json=self.new_question_missing_data)
        data = json.loads(res.data)
    ...
    def test_get_question_search_with_results(self):
        res = self.client()
        .post("/questions/search", json={"search_term": "tom"})
        data = json.loads(res.data)
    ...
    def test_get_book_search_without_results(self):
        res = self.client()
        .post("/questions/search", json={"search_term": "personal"})
        data = json.loads(res.data)
    ...
    def test_400_payload_error_in_quizzes(self):
        res = self.client().post(
            ...
            json={
                "previous_question": [],
                "quiz_categories": {"id": "1", "type": "Science"},
            },
        )
        data = json.loads(res.data)
    ...
    def test_422_if_proper_category_format_not_provided__in_quizzes(self):
        res = self.client().post(
            "/quizzes", json={"previous_questions": [], "quiz_category": "4"}
        )
        data = json.loads(res.data)
